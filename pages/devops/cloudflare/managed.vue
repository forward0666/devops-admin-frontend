...
async function fetchAllRules() {
  ...
  try {
    ...
    for (const aid of accountIds) {
      try {
        const { data } = await apiClient.get(`${CF_GATEWAY}/managed`, { params: { account_id: aid } })
...
async function syncZone(zoneId: string) {
  ...
  try {
    ...
    const { data } = await apiClient.post(
      `${CF_GATEWAY}/zones/${zoneId}/managed/sync`,
      null,
  ...
  try {
    ...
    for (const [aid, zs] of Object.entries(accountZones)) {
      ...
      for (const z of zs) {
        try {
          const { data } = await apiClient.post(
            `${CF_GATEWAY}/zones/${z.zone_id}/managed/sync`,
            null,
...
async function fetchRules(zoneId: string) {
  if (!selectedAccountId.value) return
  try {
    const { data } = await apiClient.get(`${CF_GATEWAY}/zones/${zoneId}/managed`, {
...
function exportCSV() {
  ...
  a.href = url
  a.download = 'managed_rules_' + new Date().toISOString().slice(0, 10) + '.csv'
  a.click()
  ...
  <div style="display: flex; flex-direction: column; flex: 1; min-height: 0;">
            ...
            <tr class="cursor-pointer" @click="toggleZone(z.zone_id)" style="background: rgb(var(--v-theme-on-surface), 0.04);">
              <td colspan="6" style="padding: 0 !important;">
                <div class="d-flex align-center" style="padding: 12px 16px;">
                  ...
                  <VIcon icon="bx-shield" size="18" class="me-2 text-medium-emphasis" />
    ...
    <VCard v-else>
      <VCardText class="text-center py-8 text-medium-emphasis">
        <VIcon icon="bx-shield" size="48" class="mb-2" />
        <p>Select an account to manage Managed Rules</p>
      </VCardText>
    </VCard>
