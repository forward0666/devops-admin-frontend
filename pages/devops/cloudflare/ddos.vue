...
const actionColors: Record<string, string> = { block: 'error', allow: 'success', challenge: 'warning', skip: 'info', managed_challenge: 'info', log: 'grey' }
...
async function fetchAllRules() {
  ...
  try {
    ...
    for (const aid of accountIds) {
      try {
        const { data } = await apiClient.get(`${CF_GATEWAY}/ddos`, { params: { account_id: aid } })
    ...
    Object.keys(map).forEach(zid => {
      map[zid].sort((a: any, b: any) => (a.position_index ?? 99999) - (b.position_index ?? 99999))
    })
    rulesMap.value = map
  } catch (e: any) {
...
async function syncZone(zoneId: string) {
  ...
  try {
    ...
    const { data } = await apiClient.post(
      `${CF_GATEWAY}/zones/${zoneId}/ddos/sync`,
      null,
...
async function fetchRules(zoneId: string) {
  ...
  if (!aid) return
  try {
    const { data } = await apiClient.get(`${CF_GATEWAY}/zones/${zoneId}/ddos`, {
...
function toggleZone(zoneId: string) {
  ...
  if (next && !rulesMap.value[zoneId]) fetchRules(zoneId)
}
...
const filteredZones = computed(() => {
  ...
  return [...list].sort((a, b) => {
    ...
    const ap = aRules.length > 0 ? Math.min(...aRules.map((r: any) => r.position_index ?? 99999)) : 99999
    const bp = bRules.length > 0 ? Math.min(...bRules.map((r: any) => r.position_index ?? 99999)) : 99999
    return ap - bp
  })
})
...
function exportCSV() {
  ...
  a.href = url
  a.download = 'ddos_rules_' + new Date().toISOString().slice(0, 10) + '.csv'
  a.click()
  ...
  <div style="display: flex; flex-direction: column; flex: 1; min-height: 0;">
    <VCard class="mb-4">
      <VCardText class="d-flex align-center flex-wrap gap-3 py-3">
        ...
        <VTextField v-model="search" prepend-inner-icon="bx-search" placeholder="Filter by domain..." density="compact" hide-details clearable style="max-width: 260px" />
    ...
    <VCard v-if="selectedAccountId" style="display: flex; flex-direction: column; flex: 1; min-height: 0;">
      ...
      <VTable v-if="zones.length > 0" class="text-no-wrap sticky-table" hover density="compact" style="flex: 1; min-height: 0; table-layout: fixed; width: 100%;">
        ...
        <thead>
          <tr class="text-caption text-medium-emphasis">
            <th style="width: 100px !important; max-width: 100px !important;">Zone</th>
            <th style="width: 170px !important; max-width: 170px !important;">Action</th>
            <th>Description</th>
            <th>Position</th>
            <th>Status</th>
            <th>Synced</th>
            ...
            <tr class="cursor-pointer" @click="toggleZone(z.zone_id)" style="background: rgb(var(--v-theme-on-surface), 0.04);">
              <td colspan="6" style="padding: 0 !important;">
                <div class="d-flex align-center" style="padding: 12px 16px;">
                  ...
                  <VIcon icon="bx-shield-x" size="18" class="me-2 text-medium-emphasis" />
            ...
            <template v-if="expandedZones[z.zone_id]">
              <template v-if="(rulesMap[z.zone_id] || []).length > 0">
                <tr v-for="r in rulesMap[z.zone_id]" :key="r.rule_id">
                  <td><div style="padding-left: 36px;" class="font-weight-medium">{{ z.name }}</div></td>
                  <td><VChip size="x-small" :color="actionColors[r.action] || 'grey'" variant="tonal">{{ r.action }}</VChip></td>
                  <td><div class="font-weight-medium text-caption">{{ r.description || '-' }}</div></td>
                  <td class="text-caption text-center">{{ r.position_index ?? '-' }}</td>
                  <td><VChip size="x-small" :color="r.enabled !== false ? 'success' : 'grey'" variant="tonal">{{ r.enabled !== false ? 'active' : 'disabled' }}</VChip></td>
                  <td class="text-caption text-medium-emphasis">{{ r.synced_at ? new Date(r.synced_at + 'Z').toLocaleString('zh-CN', { hour12: false, timeZone: 'Asia/Shanghai' }) : '-' }}</td>
                </tr>
              </template>
              <template v-if="(rulesMap[z.zone_id] || []).length === 0">
                <tr><td colspan="6" class="text-center py-4 text-medium-emphasis">No synced rules. Click Sync to fetch from Cloudflare.</td></tr>
              </template>
            </template>
          </template>
        </tbody>
      ...
      <VCardText v-else-if="!loadingZones" class="text-center py-8 text-medium-emphasis">
        <VIcon icon="bx-shield-x" size="48" class="mb-2" />
    ...
    <VCard v-else>
      <VCardText class="text-center py-8 text-medium-emphasis">
        <VIcon icon="bx-shield-x" size="48" class="mb-2" />
        <p>Select an account to manage DDoS rules</p>
      </VCardText>
    </VCard>
...
.sticky-table :deep(.v-table__wrapper) { flex: 1; min-height: 0; overflow-y: auto; max-height: calc(100vh - 200px); }
