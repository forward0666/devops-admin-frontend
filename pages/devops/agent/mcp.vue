...
async function fetchItems() {
  loading.value = true
  try {
    const { data } = await apiClient.get(`${AGENT_GATEWAY}/mcps`)
...
async function save() {
  ...
  try {
    if (editingId.value) {
      await apiClient.put(`${AGENT_GATEWAY}/mcps/${editingId.value}`, form.value)
      snackbar.value = { show: true, text: 'MCP updated', color: 'success' }
    } else {
      await apiClient.post(`${AGENT_GATEWAY}/mcps`, form.value)
      snackbar.value = { show: true, text: 'MCP created', color: 'success' }
    }
    dialog.value = false
    await fetchItems()
  } catch (e: any) {
...
async function handleDelete(item: any) {
  ...
  try {
    await apiClient.delete(`${AGENT_GATEWAY}/mcps/${item.id}`)
    snackbar.value = { show: true, text: 'MCP deleted', color: 'success' }
    await fetchItems()
  } catch (e: any) {
    snackbar.value = { show: true, text: e?.response?.data?.detail || 'Failed', color: 'error' }
  }
...
async function toggleEnabled(item: any) {
  try {
    await apiClient.patch(`${AGENT_GATEWAY}/mcps/${item.id}`, { enabled: !item.enabled })
  ...
  <div style="display: flex; flex-direction: column; flex: 1; min-height: 0;">
    <VCard class="mb-4">
      <VCardText class="d-flex align-center flex-wrap gap-3 py-3">
        <div class="flex-grow-1">
          <h4 class="text-h4 mb-1">MCP Management</h4>
          <p class="text-body-2 text-medium-emphasis mb-0">Manage Model Context Protocol servers</p>
        </div>
        <VBtn color="primary" @click="openCreate">
          <VIcon icon="bx-plus" class="me-1" /> Add MCP
        </VBtn>
      </VCardText>
    </VCard>
    ...
    <VCard style="display: flex; flex-direction: column; flex: 1; min-height: 0;">
      ...
      <VCardText v-else-if="!loading" class="text-center py-8 text-medium-emphasis">
        ...
        <p>No MCP servers yet. Click "Add MCP" to create one.</p>
      </VCardText>
    </VCard>

    <VDialog v-model="dialog" max-width="600">
      <VCard>
        <VCardTitle>{{ editingId ? 'Edit MCP' : 'Add MCP' }}</VCardTitle>
        <VDivider />
        <VCardText>
          <VTextField v-model="form.name" label="MCP Name" class="mb-3" variant="outlined" hint="e.g. mcp-weather, mcp-k8s" persistent-hint />
