...
export function useAppShell(options: AppShellOptions = {}) {
  ...
  function handleMessage(event: MessageEvent) {
    ...
    switch (data.type) {
      case 'INIT':
        // Receive token from parent shell
        if (data.data?.token) {
  ...
  function notifyReady() {
    postToShell({ type: 'READY', data: { app: 'iam-console' } })
  }

  function notifyError(error: string) {
    postToShell({ type: 'app:error', payload: { app: 'iam-console', error } })
  }
